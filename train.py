...
def sample_masked_block(true_tokens, true_mask, block_size, mask_token_id):
    """Pick one random block per batch row; hybrid mask schedule.

    Per row, with prob 0.8 mask all positions 1..B-1 (matches eval/inference
    exactly). With prob 0.2 use a random p~U(0,1) and mask each non-seed
    position iid with prob p (regularizer against fixed-pattern overfitting).
    Returns (block_ids, block_targets, block_valid, was_masked, starts).
    """
    ...
    use_random = torch.rand(B, 1, device=device) < 0.2
    p = torch.where(use_random, torch.rand(B, 1, device=device), torch.ones(B, 1, device=device))
    mask_pattern = torch.rand(B, block_size - 1, device=device) < p
